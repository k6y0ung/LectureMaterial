...
export default {
  ...
  methods: {
    ...
    addTodo(newTodoItem) {
      ...
      const newTodo = { id: maxid + 1, todo: newTodoItem, done: false };
    ...
    doneToggle(id) {
      // const newTodo = this.$data.todoItems.map((item) => {
      //   debugger;
      //   if (item.id === id) {
      //     item.done = !item.done;
      //   }
      //   return item;
      // });
      //   this.$data.todoItems = newTodo;
      this.dispatchDoneToggle(id);
    },
    removeTodo(id) {
      const newTodos = this.$data.todoItmes.filter((item) => {
        debugger;
        if (item.id === id) {
          return false;
        }
